...
import { GenerateFrSubstractionsDto } from 'src/modules/maths/dtos/fractions/generate-fr-substractions.dto';

export class FrSubstractionsRequestBuilder {
  static buildMessages(
    generateFrAdditionsDto: GenerateFrSubstractionsDto,
  ): ChatCompletionMessageParam[] {
    ...
    const messages: ChatCompletionMessageParam[] = [
       ...
       */
      {
        role: 'user',
        content: `
        Devuelve ${quantity} operaciones de resta de fracciones las cuales deben de cumplir con todos y cada uno de los siguientes criterios:
        * Los numeros de las fracciones no deben ser 0.
        * ${generateFrAdditionsDto.canHaveWholes ? 'El 50% de las restas deben contener fracciones ser mixtas, ambas fracciones pueden ser mixtas o solo una de ellas.' : 'Las fracciones no deben ser mixtas'}.
        * ${FrSubstractionsRequestBuilder.buildDenominatorsTypesRules(generateFrAdditionsDto)}.
        * Las fracciones deben ser propias.
        * Las fracciones deben ser diferentes entre si en las ${quantity} restas.
  ...
  static buildSchema(): ResponseFormatJSONSchema.JSONSchema {
    return {
      name: 'fr_substractions_response',
      strict: true,
      schema: {
        type: 'object',
        properties: {
